...
function init() {
    // jQuery.get("http://120.24.245.188:2100/token/" + requestToken, function (data, status) {
    //     if (data && data.main) {
    //         //call play
    //         play(data.main, data.xml)
    //     }
    // });
    play('/ppvod/5xvmXj2Z.m3u8', '/ppvod/jkuyVBIx');
}
...
function play(main, xml) {
    ...
    var url = 'http://120.24.245.188:2100' + main


    // xml = window.location.protocol + "//" + window.location.host + xml
    xml = 'http://120.24.245.188:2100' + xml
    ...
    CKobject.embed('/static/ckplayer/ckplayer.swf', 'a1', 'ckplayer_a1', '100%', '100%', false, flashvars, video, params);
